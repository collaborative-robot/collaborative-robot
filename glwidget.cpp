...
GLWidget::GLWidget(QWidget *parent) :
    QGLWidget(parent),
    pitch(30.0), yaw(0.0), distance(2)//默认参数
{
}
...
void GLWidget::paintGL()
{
    ...
    glBegin(GL_LINES);
    glLoadIdentity();
        glVertex3f(-robot->getY(), robot->getDL()+robot->getX(), 0.0);
        glVertex3f(-robot->getY(), robot->getDL()+robot->getX(), robot->getZ()+0.5);//系统建模坐标系不等于参数基坐标系
    glEnd();

    glColor4f(0.9, 0.0, 0.0, 0.5);
    //robot->displayPath();//显示末端，没必要
